...
app.post('/api/user/:telegram_id/report', async (req, res) => {
  ...
  const client = await pool.connect();
  try {
    // 1) دریافت کاربر + لاگ‌ها + تاریخچه پریود
    ...
    const logs = logsRes.rows.filter(r => {
      const d = moment(r.log_date, 'YYYY-MM-DD');
      return d.isSameOrAfter(startG, 'day') && d.isSameOrBefore(endG, 'day');
    });
    ...
    const avgCycleLen = Math.round(user.avg_cycle_length || user.cycle_length || 28);
    const cycleLenSafe = Math.max(21, Math.min(60, avgCycleLen));
    const pmsWindow = 4;
